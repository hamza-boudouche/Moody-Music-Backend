...
def getMusic(moodid: str):
	...
	for idx, playlist in enumerate(playlists):
		score = Score.query.filter_by(uri=playlist.uri, user=playlist.user).first() # does this work ?!
		if score is None:
			continue
		if score.score == 1:
...
def addMusic(): 
	...
	if not validate.validate_link('https://open.spotify.com/album/' + content.get('uri').strip()):
	...
	if Playlist.query.filter_by(uri=content.get('uri')).count() != 0:
		return {'success': False, 'message': 'invalid uri (already bound to another playlist instance)'}, 409
	if genre is None:
...
def vote():
	...
	if content.get('uri') is None or content.get('vote') is None or content.get('moodid') is None:
...
def update(uri):
	...
	if playlist is None:
		return {'success': False, 'message': 'playlist not found or user not authorized to update it'}, 404
	content = request.get_json()
