...
from app.utils import crypto, auth, validate, mail, COOKIE_NAME, DEFAULT_COUNT, CACHE_TIMEOUT, DEFAULT_PLAYLIST_HISTORY_COUNT
from app import db
import logging
...
def getUser():
	...
	noPlaylists = Playlist.query.filter_by(owner=user)
	recentlyUpvotedScores = Score.query.filter_by(user=user, score=1).order_by(desc(Score.date)).limit(DEFAULT_PLAYLIST_HISTORY_COUNT).all()
	recentlyUpvotedPlaylists = [score.playlist for score in recentlyUpvotedScores]
	return {**user.toDict(), "number of upvotes": noUpvotes, "number of downvotes": noDownvotes, "number of playlists": noPlaylists, "recently upvoted playlists": recentlyUpvotedPlaylists}, 200
...
def getUsers():
	...
	for user in users:
		...
		recentlyUpvotedScores = Score.query.filter_by(user=user, score=1).order_by(desc(Score.date)).limit(DEFAULT_PLAYLIST_HISTORY_COUNT).all()
		recentlyUpvotedPlaylists = [score.playlist for score in recentlyUpvotedScores]
		res.append({**user.toDict(), "number of upvotes": noUpvotes, "number of downvotes": noDownvotes, "number of playlists": noPlaylists, "recently upvoted playlists": recentlyUpvotedPlaylists})
